...
class ManageProgram(QObject):
    ...
    def __init__(self):
        ...
        self.happy2_thread = happy2.Happy2Thread(self)
        self.happy2_thread.daemon = True
        self.happy2_thread.start()

        # self.detector_thread = detector.DetectorThread(self)
        ...
        # self.analyze_thread = analyze.AnalyzeThreadThread(self)
        ...
        #self.capture_thread.detector_start_signal.connect(self.detector_thread.Detector_slot)
        #self.analyze_thread.save_data_signal.connect(self.save_data_thread.save_data_slot)
        #self.detector_thread.analyze_signal.connect(self.save_data_thread.save_data_slot)
        self.capture = []

        #self.monitor_2_ui.begin_signal.connect(self.begin_slot)
        ...
        self.capture_thread.save_information_signal_1.connect(self.save_data_thread.infomation_slot)
        self.capture_thread.save_information_signal_1.connect(self.happy_thread.save_image)
        self.capture_thread.save_information_signal_1.connect(self.monitor_1_ui.set_text)
        self.capture_thread.save_information_signal_2.connect(self.save_data_thread.infomation_slot)
        self.capture_thread.save_information_signal_2.connect(self.happy2_thread.save_image)
        self.capture_thread.save_information_signal_2.connect(self.monitor_1_ui.set_text)

    def start_capture_slot(self):
        print("capture_start")
        self.capture_thread.frame_count = 0
        print(f"is capture:{self.IsCapture}")
        self.save_data_thread.make_path
    ...
    def stop_capture_slot(self):
        ...
        if(self.current_phase == "No Capture" or self.current_phase == None):
    ...
    def end_slot(self):
        ...
        self.current_phase = None
        self.current_frame_no = 0
        self.save_data_thread.diameters = [[0], [0], [0]]
